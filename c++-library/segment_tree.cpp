...
template<typename T> struct SegmentTree {
  ...
  SegmentTree(int _N) : _N(_N) {
    ...
    while (N < _N)
      N <<= 1;
    dat.assign(2 * N - 1, INF); // rewrite here
  }
  ...
  void update(int k, T val) {
    ...
    dat[k] = val; // rewrite here

    while (k > 0) {
      k = (k - 1) / 2;
      dat[k] = min(dat[2 * k + 1], dat[2 * k + 2]);  // rewrite here
    }
  }
  ...
  T query(int a, int b, int k, int l, int r) {
    if (r <= a || b <= l) return INF; // rewrite here
    if (a <= l && r <= b) return dat[k];

    int mid = (l + r) / 2;
    return min(query(a, b, 2 * k + 1, l, mid),
               query(a, b, 2 * k + 2, mid, r));  // rewrite here
  }
};
