...
when isMainModule:
  let
    (N, P) = input(int, 2)
  var
    oyatu = newseq[(int, int)](N)
  for i in 0..<N:
    let (a, b) = input(int, 2)
    oyatu[i] = (a, b)
  oyatu.sort do (x, y: (int, int)) -> int:
    result = cmp(-x[0], -y[0])

  var ans = 0
  var dp = newSeqWith(2, newSeqWith(P+1, 0))
  for i, o in oyatu:
    let
      (price, value) = o
      (now_i, next_i) = (i and 1, (i+1) and 1)
    for p in 0..P:
      if p - price >= 0:
        dp[next_i][p] = dp[now_i][p].max(dp[now_i][p - price] + value)
      else:
        dp[next_i][p] = dp[now_i][p]
      ans = ans.max(dp[next_i][p].max(dp[now_i][p] + value))
  ans.echo
