...
struct Edge {
  ...
  Edge(int src, int dst, T weight1, T weight2) : src(src), dst(dst), weight1(weight1), weight2(weight2) {}
};
...
struct MinimumRatioSpanningTree {
  ...
  T run_mst(double &fcost, double &gcost) {
    ...
    sort(g.E.begin(), g.E.end(), [&](Edge<T> e1, Edge<T> e2) {
      return e1.weight1 - mincost * e1.weight2 < e2.weight1 - mincost * e2.weight2; });
    DisjointSet uf(g.V);
    for (auto&& e : g.E) {
      if (e.weight1 - mincost * e.weight2 <= 0.0 || !uf.same(e.src, e.dst)) {
  ...
  T run() {
    ...
    for (mincost = 0.0; ; mincost = fcost / gcost) {
      if (std::fabs(run_mst(fcost, gcost)) < 1e-3) {
        break;
      }
    }
    return mincost;
...
int main() {
  ...
  Graph<double> g(N);
  REP(i, M) {
    int a, b; double c, t; cin >> a >> b >> c >> t;
    g.add_edge(a, b, c, t);
  }
