...
proc rec(i, j, turn: int, dp: var seq[seq[int]], A: seq[int]): int =
  if i + j == A.len:
    return 0
  if dp[i][j] != INF:
    return dp[i][j]
  ...
  if turn == 0:
    result = max(rec(i + 1, j, turn xor 1, dp, A) + A[i],
                 rec(i, j + 1, turn xor 1, dp, A) + A[A.len - j - 1])
  else:
    result = min(rec(i + 1, j, turn xor 1, dp, A) - A[i],
                 rec(i, j + 1, turn xor 1, dp, A) - A[A.len - j - 1])
  dp[i][j] = result
...
when isMainModule:
  let
    N = input(int, 1)
    A = input(int)
  var dp = newSeqWith(N, newSeqWith(N, INF))
  rec(0, 0, 0, dp, A).echo
