  ...
  for state in 0..<(1 shl D):
    ...
    for i in 0..<D:
      if (state shr i and 1) == 1:
        cnt += probs[i]
        score -= probs[i] * 100 * (i + 1) + bonus[i]
    for i in countDown(D-1, 0):
      let prob_score = 100 * (i + 1)
      if score <= 0:
        break
      if (state shr i and 1) == 1:
        continue
      if score <= probs[i] * prob_score:
        cnt += (score.float / prob_score.float).ceil.int
        score -= (score.float / prob_score.float).ceil.int * prob_score
        break
      score -= probs[i] * prob_score + bonus[i]
      cnt += probs[i]
    ans = min(ans, cnt)
  ans.echo
