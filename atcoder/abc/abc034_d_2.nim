...
when isMainModule:
  ...
  var t = 50.0
  while true:
    salines.sort do (x, y: (int, int)) -> int:
      result = cmp(-(x[1].float - t) * x[0].float, -(y[1].float - t) * y[0].float)
    var
      fcost = salines[0..<K].mapIt(it[0] * it[1]).sum.float
      gcost = salines[0..<K].mapIt(it[0]).sum.float
    if abs(fcost - gcost * t) < 1e-3:
      break
    t = fcost / gcost
  t.formatFloat(precision=32).echo
