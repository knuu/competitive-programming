...
when isMainModule:
  let
    (N, M) = input(int, 2)
  var keys = newseq[(int, int)](M)
  for i in 0..<M:
    let (a, b) = input(int, 2)
    var key = 0
    for c in input(int):
      key = key or (1 shl (c - 1))
    keys[i] = (a, key)

  var dp = newSeqWith(1 shl N, INF)
  dp[0] = 0
  for i in 0..<(1 shl N):
    if dp[i] == INF:
      continue
    for key_tmp in keys:
      let (cost, key) = key_tmp
      let next_key = i or key
      dp[next_key] = min(dp[next_key], dp[i] + cost)
  if dp[^1] == INF:
    echo(-1)
  else:
    echo(dp[^1])
