...
when isMainModule:
  let
    (N, K) = input(int, 2)
  var salines = newseq[(int, int)](N)
  for i in 0..<N:
    salines[i] = input(int, 2)
  var (left, right) = (0.0, 100.0)
  while (right - left).abs > 1e-5:
    let t = (left + right) / 2
    var arr = newseq[float](N)
    for i, saline in salines:
      let (w, p) = saline
      arr[i] = (p.float - t) * w.float
    arr.sort(cmp)
    if arr[^K..^1].sum >= 0.0:
      left = t
    else:
      right = t
  left.formatFloat(precision=32).echo
