...
when isMainModule:
  ...
  for i in A..B:
    if msum * i < mcnt * accs[i]:
      (msum, mcnt) = (accs[i], i)
      cand = newseq[int]()
      cand.add(i)
    elif msum * i == mcnt * accs[i]:
      cand.add(i)
  (msum / mcnt).formatFloat(precision = 32).echo


  var pascal = newSeqWith(N+1, newSeqWith(N+1, 1))
  for i in 1..N:
    for j in 1..<i:
